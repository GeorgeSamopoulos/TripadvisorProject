 ...
 */
class WP_TripAdvisor_Review_Activator {
	...
	public static function activate() {
		...
		$paidoptions = get_option( 'wptripadvisor_options' );
		$freeoptions = get_option( 'wpfbr_options' );
		if(!$paidoptions && $freeoptions){
			update_option( 'wptripadvisor_options', $freeoptions );
		}
		

		//setup cron to get tripadvisor once a day
		if (! wp_next_scheduled ( 'wptripadvisor_daily_event' )) {
			wp_schedule_event(time(), 'daily', 'wptripadvisor_daily_event');
		}
	}
